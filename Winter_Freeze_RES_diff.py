...
async def load_bot_usernames():
    """Загружает username всех ботов (основной + зеркала) при старте"""
    ...
    try:
        ...
        all_bot_usernames.add(me.username.lower() if me.username else None)
    ...
    for uid, data in users.items():
        for token in data.get('tokens', []):
            ...
            try:
                ...
                logger.warning(f"❌ Токен unauthorized: {token[:15]}... Удаляем из БД")
                await remove_invalid_token(token)
            except Exception as e:
                logger.error(f"Ошибка получения username зеркала {token[:15]}...: {e}")
    ...
    async with db_lock:
        if not os.path.exists(DB_FILE):
            with open(DB_FILE, "w", encoding="utf-8") as f:
                f.write("id|name|sub_until|reports|tokens|requests\n")
            return
        ...
        if lines and "requests" not in lines[0]:
            logger.info("Выполняется миграция базы данных...")
    ...
    async with db_lock:
        with open(DB_FILE, "w", encoding="utf-8") as f:
            ...
            for uid, data in users.items():
                ...
                f.write(f"{uid}|{data['name']}|{data['sub_until']}|{data['reports']}|{tokens_str}|{requests}\n")
...
import asyncio
import aiohttp
import socks  # Нужен для Telethon
import sys
...
async def main():
    ...
    if not await check_proxy():
        logger.critical("Запуск невозможен. Проверьте сетевое подключение.")
        sys.exit(1)

    await init_db()

    # === ЗАГРУЗКА USERNAME ВСЕХ БОТОВ ===
    await load_bot_usernames()

    logger.info("Инициализация Telethon напрямую...")
    ...
    if hasattr(main_bot, '_session') and main_bot._session is not None:
        await main_bot._session.close()
    main_bot._session = custom_session

    mirror_bots = await load_all_mirrors()
    for bot in mirror_bots:
        if hasattr(bot, '_session') and bot._session is not None:
            await bot._session.close()
        bot._session = custom_session

    all_bots = [main_bot] + mirror_bots

    logger.info(f"🚀 Запущено ботов: {len(all_bots)} | Защищено username: {len(all_bot_usernames)}")
